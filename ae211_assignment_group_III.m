...
% Recovering tangential velocity at the control points for Cp calculation
...
if(shape=='batman')
    ...
    % h(5) = quiver(midpoint_x,midpoint_y,V.*cos(phi+alpha),V.*sin(phi+alpha),'DisplayName','Velocity at the surface');
else
    ang=0:0.01:2*pi; 
    cir_x=R*cos(ang);
    cir_y=R*sin(ang);
